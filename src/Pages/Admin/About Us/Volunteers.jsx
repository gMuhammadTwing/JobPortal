...
export default function Index() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoader(true)
            try {
                const response = await axiosInstance.post(`/api/veritas_kwd_volunteers/store`, values);
                if (response) {
                    toast.success("Volunteer Data saved successfully");
                }
            } catch (error) {
                console.error(error);
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const { data } = await axiosInstance.get(`/api/veritas_kwd_volunteers`);
            if (data) {
                setData(data[0]);
            }
        } catch (error) {
            ...
            <div className="pb-15">
                <>
                    <div className="border rounded-lg p-4 bg-white">
                        <div className="text-center p-4 font-semibold">VeritasKWD Volunteers</div>
                        {!loading &&
