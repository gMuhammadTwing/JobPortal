...
export default function Opportunity() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoader(true)
            try {
                const response = await axiosInstance.post(`/api/veritas_kwd_opportunity/store`, values);
                if (response) {
                    toast.success("Opportunity Data saved successfully");
                }
            } catch (error) {
                console.error(error);
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const { data } = await axiosInstance.get(`/api/veritas_kwd_opportunity`);
            setData(data[0]);
        } catch (error) {
            handleError(error);
        } finally {
