...
export default function Index() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoading(true)
            try {
                const response = await axiosInstance.post(`/api/veritas_kwd_charities/store`, values);
                if (response) {
                    toast.success("Charities Data saved successfully");
                }
            } catch (error) {
                console.error(error);
    ...
    const fetchData = async () => {
        try {
            const { data } = await axiosInstance.get(`/api/veritas_kwd_charities`);
            if (data) {
                setData(data[0]);
            }
        } catch (error) {
            ...
            <div className="pb-15">
                <>
                    <div className="border rounded-lg p-4 bg-white">
                        <div className="text-center p-4 font-semibold">VertiasKWD Charities</div>
